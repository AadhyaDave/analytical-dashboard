...
import { ChevronRight, AlertCircle, CheckCircle2, X, Settings } from 'lucide-react';
import DrilldownBreadcrumb from '../../components/shared/DrilldownBreadcrumb';
import OEEGauge from '../../components/shared/OEEGauge';

// Use the local mock data representing the higher-stakes executive prototype
import {
      ...
      {ranked.map((plant, i) => {
        ...
        return (
          <div
            ...
            onClick={() => drillDown && drillDown(plantName, { plantIdx: plantPerformanceTable.indexOf(plant) })}
          >
...
const PlantModule = ({ plant, index, drillDown, onInvestigate }) => {
      ...
      <div className="p-6 flex flex-wrap lg:flex-nowrap items-center justify-between gap-4 lg:gap-8 bg-[var(--bg-card)]">
        ...
        <div className="flex items-center gap-4 lg:gap-8 min-w-max">
          ...
          <div className="grid grid-cols-2 gap-y-3 gap-x-6">
            <div className="ops-hover-surface p-2 rounded cursor-pointer -m-2" onClick={() => drillDown && drillDown(plantName, { plantIdx: index, context: 'availability' })}>
            ...
            <div className="ops-hover-surface p-2 rounded cursor-pointer -m-2" onClick={() => drillDown && drillDown(plantName, { plantIdx: index, context: 'performance' })}>
            ...
            <div className="ops-hover-surface p-2 rounded cursor-pointer -m-2" onClick={() => drillDown && drillDown(plantName, { plantIdx: index, context: 'quality' })}>
            ...
            <div className="ops-hover-surface p-2 rounded cursor-pointer -m-2" onClick={() => drillDown && drillDown(plantName, { plantIdx: index, context: 'downtime' })}>
...
/* ------------------------------------------------------------------ */
/* Inline Investigation Workspace                                     */
/* ------------------------------------------------------------------ */
const InvestigationWorkspace = ({ plantIdx, onClose, drillDown }) => {
  ...
  const plantName = nameParts[0];
  const plantLocation = nameParts[1] || '';
  // Fix plantKey for this demo data
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h2 className="text-[20px] font-bold flex items-center gap-3 text-[var(--text-primary)]">
            Investigating: {plantName}
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => { onClose(); drillDown && drillDown(plantName, { plantIdx }); }}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--blue)] text-white rounded text-[11px] font-bold tracking-wide uppercase hover:bg-blue-600 transition-colors shadow-sm"
          >
      ...
      <div className="p-6 md:p-8 grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10 rounded-[var(--r-xl)] border border-[var(--border)] bg-[var(--bg-card)]">
        ...
        <div className="col-span-1 lg:col-span-2">
          ...
          <div className="space-y-3">
            {plantAlerts.map((alert, i) => (
              <div
                key={i}
                className="flex items-start gap-4 p-4 rounded border border-[var(--border-light)] bg-[var(--bg-inset)] ops-investigation-row cursor-pointer"
                onClick={() => { onClose(); drillDown && drillDown(alert.targetLabel, alert.contextData); }}
              >
        ...
        <div className="flex flex-col">
          ...
          <div
            ...
            onClick={() => { onClose(); drillDown && drillDown(plantName, { plantIdx, context: 'trend' }); }}
          >
            <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
              <LineChart
                data={plantOEETrend[plantIdx].filter(d => d.oee != null)}
                margin={{ top: 5, right: 10, left: -25, bottom: 0 }}
                onClick={(e) => { onClose(); drillDown && drillDown(plantName, { plantIdx, context: 'trend', point: e?.activeLabel }); }}
              >
        ...
        <div className="col-span-1 pt-2">
          ...
          <div className="space-y-2.5 mt-1">
            {topPerformingMachines[plantIdx].map((m, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-3 rounded bg-[var(--bg-inset)] ops-investigation-row cursor-pointer"
                onClick={() => { onClose(); drillDown && drillDown(m.name, { plantIdx, machine: m.name }); }}
              >
                <div className="flex items-center gap-2.5">
        ...
        <div className="flex flex-col pt-2">
          ...
          <div className="space-y-1.5 mt-1">
            {shiftProductivityComparison.map((shift, i) => {
              const plantKeyDynamic = plantName === 'Plant A' ? 'plantA' : plantName === 'Plant B' ? 'plantB' : plantName === 'Plant C' ? 'plantC' : 'plantD';
              const val = shift[plantKeyDynamic] || 0;
              return (
                <div
                  key={i}
                  className="flex items-center gap-4 p-2 -mx-2 rounded ops-investigation-row bg-transparent cursor-pointer"
                  onClick={() => { onClose(); drillDown && drillDown(plantName, { plantIdx, shift: shift.shift }); }}
                >
        ...
        <div className="col-span-1 pt-2">
          ...
          <div className="space-y-3.5">
            {plantLosses.slice(0, 2).map((loss, i) => (
              <div
                key={i}
                className="p-4 rounded border border-[var(--border-light)] bg-[var(--bg-inset)] ops-investigation-row flex flex-col justify-between cursor-pointer"
                onClick={() => { onClose(); drillDown && drillDown(plantName, loss.contextData); }}
              >
...
/* ------------------------------------------------------------------ */
/* Executive MD Dashboard Prototype                                      */
/* ------------------------------------------------------------------ */
const ExecutiveMDDashboard = () => {
  const [investigatingPlantIdx, setInvestigatingPlantIdx] = useState(0);
  ...
  return (
    <div className="p-6 md:p-8 space-y-5 pb-8 h-screen overflow-y-auto" style={{ background: 'var(--bg-app)' }}>
      ...
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-[var(--border)]">
        <div>
          <h1 className="text-2xl font-black text-[var(--text-primary)] flex items-center gap-3">
            ...
            Managing Director screen V.2
          </h1>
        </div>
        <div className="px-3 py-1.5 rounded-full text-xs font-bold tracking-widest flex items-center gap-2 border"
          style={{
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-col xl:flex-row items-stretch gap-4">
          ...
          <div
            ...
            onClick={() => mockDrillDown('All Plants', { context: 'global_oee' })}
          >
          ...
          <div className="flex flex-1 gap-4 min-w-0">
            {[
              ...
              <div
                ...
                onClick={() => mockDrillDown('Enterprise', { context: m.label })}
              >
          ...
          <div className="ops-card p-5 min-w-[200px] flex flex-col justify-center">
            ...
            <div className="flex-1 flex flex-col justify-center">
              <BestPerformingPlants drillDown={mockDrillDown} />
            </div>
          </div>
          ...
          <div
            ...
            onClick={() => mockDrillDown('Enterprise', { context: 'global_machine_status' })}
          >
            <span className="text-[11px] tracking-widest uppercase font-bold text-[var(--text-muted)] mb-3 text-center">All Machine Status</span>
...
export default ExecutiveMDDashboard;
